    ...
    def process_task(self, task):
        ...
        try:
            ...
            if task[0] == "set_parameter":
                ...
                entity_data = capture_screenshot(
            ...
            # Entity editing functions (using entity tokens)
                ...
                entity_data = move_body_by_token(
                    design, ui, task[1], task[2], task[3], task[4]
                )
            ...
            if task_id is not None:
                ...
                if error_message:
                    set_task_response(
                        task_id, False, f"{task_name} failed", error=error_message
                    )
                else:
                    set_task_response(
                        task_id,
            ...
            if task_id is not None:
                set_task_response(
                    task_id, False, f"{task_name} failed: {str(e)}", error_msg
                )
        ...
        entity_data["bodies"].append(
            {
                ...
                "body_name": body.name,
                "body_index": rootComp.bRepBodies.count
                - extrudeFeature.bodies.count
                + i,
            }
        )
...
def create_sphere(design, ui, radius, x, y, z):
    ...
    revInput = revolves.createInput(
        profile, axisLine, adsk.fusion.FeatureOperations.NewComponentFeatureOperation
    )
...
def draw_ellipis(
    ...
    majorAxisPoint = adsk.core.Point3D.create(
        float(x_major), float(y_major), float(z_major)
    )
    throughPoint = adsk.core.Point3D.create(
        float(x_through), float(y_through), float(z_through)
    )
...
def draw_Witzenmann(design, ui, scaling, z):
    ...
    points1 = [
        (8.283 * scaling, 10.475 * scaling, z),
        (8.283 * scaling, 6.471 * scaling, z),
        (-0.126 * scaling, 6.471 * scaling, z),
        (8.283 * scaling, 2.691 * scaling, z),
        (8.283 * scaling, -1.235 * scaling, z),
        (-0.496 * scaling, -1.246 * scaling, z),
        (8.283 * scaling, -5.715 * scaling, z),
        (8.283 * scaling, -9.996 * scaling, z),
        (-8.862 * scaling, -1.247 * scaling, z),
        (-8.859 * scaling, 2.69 * scaling, z),
        (-0.639 * scaling, 2.69 * scaling, z),
        (-8.859 * scaling, 6.409 * scaling, z),
    ...
    for i in range(len(points1) - 1):
        start = adsk.core.Point3D.create(points1[i][0], points1[i][1], points1[i][2])
            ...
            points1[i + 1][0], points1[i + 1][1], points1[i + 1][2]
        )
    ...
    points2 = [
        (-3.391 * scaling, -5.989 * scaling, z),
        (5.062 * scaling, -10.141 * scaling, z),
        (-8.859 * scaling, -10.141 * scaling, z),
        (-8.859 * scaling, -5.989 * scaling, z),
    ]
    for i in range(len(points2) - 1):
        start = adsk.core.Point3D.create(points2[i][0], points2[i][1], points2[i][2])
        end = adsk.core.Point3D.create(
            points2[i + 1][0], points2[i + 1][1], points2[i + 1][2]
        )
...
def delete_body_by_token(design, ui, body_token):
    ...
    return {"deleted_body_name": body_name, "deleted_body_token": body_token}
...
def edit_extrude_distance(design, ui, feature_token, new_distance):
    ...
    for i in range(feature.bodies.count):
        ...
        entity_data["bodies"].append(
            {"body_token": body.entityToken, "body_name": body.name}
        )
...
def get_feature_info_by_token(design, ui, feature_token):
    ...
    entity_data = {
        ...
        "feature_name": feature.name,
        "is_suppressed": (
            feature.isSuppressed if hasattr(feature, "isSuppressed") else False
        ),
        "bodies": [],
    }
...
def create_thread(design, ui, inside, sizes):
    ...
    threadInfo = threadFeatures.createThreadInfo(
        inside, threadType, threadDesignation, threadClass
    )
...
def boolean_operation(design, ui, op):
    ...
    tools.add(toolBody)
    input: adsk.fusion.CombineFeatureInput = combineFeatures.createInput(
        targetBody, tools
    )
...
def revolve_profile(design, ui, angle=360):
    ...
    input.setAngleExtent(
        False, adsk.core.ValueInput.createByString(str(angle) + " deg")
    )
...
def export_as_STEP(design, ui, Name):
    ...
    )  # Save as Fusion.step in the export directory
...
def capture_screenshot(design, ui, name, width, height, directory=None):
    ...
    return {"image_path": file_path, "width": int(width), "height": int(height)}
...
def extrude_thin(design, ui, thickness, distance):
    ...
    extInput = exts.createInput(
        selectedFace, adsk.fusion.FeatureOperations.NewBodyFeatureOperation
    )
    ...
    extInput.setOneSideExtent(
        distanceExtent, adsk.fusion.ExtentDirections.PositiveExtentDirection
    )
...
def holes(design, ui, points, width=1.0, distance=1.0, faceindex=0):
    ...
    sk = sketches.add(
        latest_body.faces.item(faceindex)
    )  # create sketch on faceindex face
        ...
        try:
            if self.path == "/count_parameters":
                self.send_json_response(
                    {
                        "success": True,
                        "user_parameter_count": len(ModelParameterSnapshot),
                    }
                )
            elif self.path == "/list_parameters":
                self.send_json_response(
                    {"success": True, "ModelParameter": ModelParameterSnapshot}
                )
            else:
        ...
        try:
            ...
            if path.startswith("/set_parameter"):
                ...
                if name and value:
                    ...
                    self.send_json_response(
                        {"success": False, "error": "Missing name or value"}, 400
                    )
                ...
                response = self.queue_task_and_wait(
                    ("capture_screenshot", name, width, height, directory)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("shell_body", thickness, faceindex)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("extrude_last_sketch", value, taperangle)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                plane = data.get("plane", "XY")
                response = self.queue_task_and_wait(
                    ("arc", point1, point2, point3, connect, plane)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                plane = data.get("plane", "XY")
                response = self.queue_task_and_wait(
                    ("draw_one_line", x1, y1, z1, x2, y2, z2, plane)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("holes", points, width, distance, faceindex)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("extrude_thin", thickness, distance)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("circular_pattern", quantity, axis, plane)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("threaded", inside, allsizes), timeout=30.0
                ...
                plane = data.get("plane", "XY")
                response = self.queue_task_and_wait(
                    ("draw_2d_rectangle", x_1, y_1, z_1, x_2, y_2, z_2, plane)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
            ...
            # Entity editing endpoints (using entity tokens)
                ...
                z = float(data.get("z", 0))
                response = self.queue_task_and_wait(
                    ("move_body_by_token", body_token, x, y, z)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                body_token = str(data.get("body_token"))
                response = self.queue_task_and_wait(
                    ("delete_body_by_token", body_token)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("delete_entity_by_token", entity_token)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("edit_extrude_distance", feature_token, new_distance)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                body_token = str(data.get("body_token"))
                response = self.queue_task_and_wait(
                    ("get_body_info_by_token", body_token)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                feature_token = str(data.get("feature_token"))
                response = self.queue_task_and_wait(
                    ("get_feature_info_by_token", feature_token)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
                ...
                response = self.queue_task_and_wait(
                    ("set_body_visibility", body_token, is_visible)
                )
                self.send_json_response(
                    response, 200 if response.get("success") else 500
                )
...
def run(context):
    ...
    try:
        ...
        customEvent = app.registerCustomEvent(
            myCustomEvent
        )  # Every 200ms we create a custom event which doesnt interfere with Fusion main thread
        onTaskEvent = (
            TaskEventHandler()
        )  # If we have tasks in the queue, we process them in the main thread
